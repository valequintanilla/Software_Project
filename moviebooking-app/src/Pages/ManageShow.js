...
const Payment = ({ shoppingCart, incrementMovieTickets }) => {
  ...
  const renderTicket = (ticket) => {
    const movie = shoppingCart.find((movie) => movie.imdbID === ticket.movieId);
    return (
      <div style={{ border: '1px solid black', padding: '10px', margin: '10px' }}>
        <h3>{movie.Title}</h3>
        <p>Theater Location: {ticket.theater}</p>
        <p>Time: {ticket.time}</p>
        <p>Seat: {ticket.seat}</p>
        <p>Name: {ticket.owner}</p>
  ...
  return (
    <div>
      ...
      {purchasedTickets.map((ticket) => renderTicket(ticket))}
